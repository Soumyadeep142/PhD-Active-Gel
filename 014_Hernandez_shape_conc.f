c Replicating Hernandez 1
      ...
      double precision bx,by,bz, c_x, c_y,c_z, eps, r, h
      double precision nuu
      parameter(Nx=16,Ny=16,Nz=16,NT=300000,snap_int=1000,c_max=2.d0)
      parameter(pi=acos(-1.d0), dt=0.01d0, grutol=0.2, lambda=0.1d0)
      parameter(r0=6,c_h=2.d0,Dc=0.1d0,As=0.5d0,Af=0.5d0)
      parameter(Du=0.02d0, sig=1.d0, alpha = 1.0d0)
      parameter(Lx=10.d0*pi, Ly=10.d0*pi, Lz=10.d0*pi, c_ini=20.d0)
...
c          u(i,j,k)=-tanh(2*(r-r0))
          
c    Cubic Initial Field 
! cube half width (distance from center to face)
           a = r0         ! use r0 as cube half-length or rename as needed

           dx = abs(dble(i) - c_x) 
           dy = abs(dble(j) - c_y) 
           dz = abs(dble(k) - c_z) 
           
           if ((dx.le.a).and.(dy.le.a).and.(dz.le.a)) then
             u(i,j,k)=1.d0
           else
             u(i,j,k)=-1.d0
           endif
       ...
       do 26 i=1, NxC
         do 27 j=1, Ny
          do 28 k=1, Nz      	    
            ...
            nuu=Du*2*sig*k2
            uh(i,j,k)=exp(-k2*nuu*dt)*(uh(i,j,k)-k2*fuh(i,j,k)*dt)
 28      continue
 27     continue
 26    continue
