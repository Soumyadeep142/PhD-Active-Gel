c This code does concentration,velocity as a field only
      ...
      parameter(pi=acos(-1.d0), dt=0.0001d0, grutol=1e-1)
      parameter(alpha=0.1d0, r0=4, c_h=0.d0)
      ...
      complex*16, allocatable :: RHSh_prev(:,:,:)
      integer*8 :: plan
      ...
      do 23 i=1,Nx
       do 24 j=1,Ny
        do 25 k=1,Nz
          ...
          if ((h.ge.c_h).and.(abs(iku(i,j,k)).ge.grutol)) then
              c(i,j,k)=1
          else
              c(i,j,k)=0
          endif
...
c**********Saving the initial file*****************************
      ...
      do 41 i=1,Nx
        do 52 j=1, Ny
         do 61 k=1,Nz
          write(8,*)i, j, k, uc(i,j,k), c(i,j,k)     
 61     continue
 52    continue
 41   continue
      close(8)
...
c      call fft_forward(Nx, Ny, Nz, vx, vxc)
c     call fft_forward(Nx, Ny, Nz, vy, vyc)
c      call fft_forward(Nx, Ny, Nz, vz, vzc)
...
c      	do 6 j=1,Ny
c      	 do 7 k=1,Nz
c      	   uh(i,j,k)=uh(i,j,k)-vuh(i,j,k)*dt
c 7       continue
c 6      continue
c 5    continue
...
c      	    cdtlndph(i,j,k)=k2*ck(i,j,k)*2*uh(i,j,k)*vuh(i,j,k)
      	 
c            ikcvxh(i,j,k)=iota*ck(i,j,k)*kx(i)*vxc(i,j,k)
c            ikcvyh(i,j,k)=iota*ck(i,j,k)*ky(j)*vyc(i,j,k)
c            ikcvzh(i,j,k)=iota*ck(i,j,k)*kz(k)*vzc(i,j,k)
            
            k2ch(i,j,k)=-k2*ck(i,j,k)
...
c            cvxh(i,j,k)=ck(i,j,k)*vxc(i,j,k)
c            cvyh(i,j,k)=ck(i,j,k)*vyc(i,j,k)
c            cvzh(i,j,k)=ck(i,j,k)*vzc(i,j,k)
 28       continue
 27     continue
 26    continue

c       call fft_backward(Nx, Ny, Nz, cdtlndph, cdtlndp)
       
c       call fft_backward(Nx, Ny, Nz, ikcvxh, ikcvx)
c       call fft_backward(Nx, Ny, Nz, ikcvyh, ikcvy)
c       call fft_backward(Nx, Ny, Nz, ikcvzh, ikcvz)
       
c       call fft_backward(Nx, Ny, Nz, k2ch, k2c)
...
c       call fft_backward(Nx, Ny, Nz, cvxh, cvx)
c       call fft_backward(Nx, Ny, Nz, cvyh, cvy)
c       call fft_backward(Nx, Ny, Nz, cvzh, cvz)

c       cdtlndp=cdtlndp*norm
...
c       ikcv(i,j,k)=sqrt(ikcvx(i,j,k)**2+ikcvy(i,j,k)**2+ikcvz(i,j,k)**2)
c       cvkz(i,j,k)=cvx(i,j,k)*kzx(i,j,k)+cvy(i,j,k)*kzy(i,j,k)+
c     1               cvz(i,j,k)*kzz(i,j,k)

       if (abs(iku(i,j,k)).ge.grutol) then            
 ...
 33    continue
       ...
       do 36 i=1,Nx
       	do 37 j=1,Ny
       	  do 38 k=1,Nz
c            RHS(i,j,k)=-cdtlndp(i,j,k)-ikcv(i,j,k)+k2c(i,j,k)-cvkz(i,j,k)
c     1                 +ikckz(i,j,k)  
             RHS(i,j,k)=ikckz(i,j,k)
 38       continue
 37     continue
 36    continue

c   Time Marching in Concentration field using Adam-Bashfort Method
       ...
       do 47 i=1, NxC
        do 48 j=1,Ny
         do 49 k=1,Nz
           RHSh(i,j,k)=RHSh(i,j,k)+k2ch(i,j,k)
 49      continue
 48     continue
 47    continue
       ...
       do 39 i=1, NxC
        do 40 j=1, Ny
          do 43 k=1, Nz
           if (it.eq.1) then
             ck(i,j,k)=ck(i,j,k)+RHSh(i,j,k)*dt
             
           else
             ck(i,j,k)=ck(i,j,k)+dt*(1.5d0*RHSh(i,j,k)
     1        -0.5d0*RHSh_prev(i,j,k))
